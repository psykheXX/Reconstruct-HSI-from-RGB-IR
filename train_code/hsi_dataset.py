...
class TrainDataset(Dataset):
    def __init__(self, data_root, crop_size, arg=True, bgr2rgb=False, stride=8):
        ...
        with open(f'../split_txt/train_list.txt', 'r') as fin:
            hyper_list = [line.replace('\n','.mat') for line in fin]
            bgr_list = [line.replace('mat','png') for line in hyper_list]
        hyper_list.sort()
        bgr_list.sort()
        print(f'len(hyper) of AG dataset:{len(hyper_list)}')
        print(f'len(bgr) of AG dataset:{len(bgr_list)}')
        for i in range(len(hyper_list)):
            ...
            with h5py.File(hyper_path, 'r') as mat:
                ...
                #print(f"Loaded hyperspectral data shape: {hyper.shape}")
                #print(f"Loaded hyperspectral data dtype: {hyper.dtype}")
                #print(f"Sample data from hyperspectral (first 5 values): {hyper.flatten()[:5]}")
            """合成数据是否要去掉这一步？"""
            ...
            bgr = cv2.imread(bgr_path, cv2.IMREAD_UNCHANGED)
            if bgr2rgb:
            ...
            mat.close()
            print(f'AG scene {i+1} is loaded.')
...
class ValidDataset(Dataset):
    def __init__(self, data_root, bgr2rgb=False):
        self.hypers = []
        self.bgrs = []
        hyper_data_path = f'{data_root}/Valid_Spec/'
        bgr_data_path = f'{data_root}/Valid_RGB/'
        with open(f'../split_txt/test_list.txt', 'r') as fin:
            hyper_list = [line.replace('\n', '.mat') for line in fin]
            bgr_list = [line.replace('mat','png') for line in hyper_list]
        hyper_list.sort()
        bgr_list.sort()
        print(f'len(hyper_valid) of AG dataset:{len(hyper_list)}')
        print(f'len(bgr_valid) of AG dataset:{len(bgr_list)}')
        for i in range(len(hyper_list)):
            hyper_path = hyper_data_path + hyper_list[i]
            if 'mat' not in hyper_path:
                continue
