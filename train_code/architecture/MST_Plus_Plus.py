...
class MST(nn.Module):
    def __init__(self, in_dim=41, out_dim=41, dim=41, stage=2, num_blocks=[2,4,4]):
...
class MST_Plus_Plus(nn.Module):
    def __init__(self, in_channels=4, out_channels=41, n_feat=41, stage=3):
        super(MST_Plus_Plus, self).__init__()
        self.stage = stage
        self.conv_in = nn.Conv2d(in_channels, n_feat, kernel_size=3, padding=(3 - 1) // 2,bias=False)
        modules_body = [MST(dim=41, stage=2, num_blocks=[1,1,1]) for _ in range(stage)]
...
if __name__ == '__main__':
    ...
    in_channels = 4
    out_channels = 41
    height, width = 128, 128  # Non-divisible by 8 to test padding
    n_feat = 41
    stage = 3
